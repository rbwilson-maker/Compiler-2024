...
let compute_live_outs (code : AS.fun_instrs) : Live_assem.fun_lines = 
  ...
  let update_uses_defs line uses = 
    let uses_op op = match op_to_dest op with
    | None -> () | Some d -> Hash_set.add uses d
    in let defs_op op = match op_to_dest op with
    | None -> `None
    | Some d -> `One d
    in match line.instr with
    | AS.Mov {dest; src} -> uses_op src; defs_op dest
    ...
    | AS.Call {params; _} -> List.iter params ~f:uses_op; (* dest should be RAX *) 
      `Many (List.map Reg.caller_saves ~f:(fun r -> Dest.Reg r))
    | AS.Branch {cond = `Single op; _} -> uses_op op; `None
