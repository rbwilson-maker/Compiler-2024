...
let compile (cmd : cmd_line_args) : unit =
  ...
  say_if cmd.dump_ast (fun () -> Print.pp_opt Ast.Print.pp_program header_ast);
  ...
  Typechecker.typecheck ~header:header_ast ast;
  ...
  let ast' = Elaborate.elaborate ~header:header_ast ast in
  ...
  let assem = Codegen.codegen ir |> Elabassem.elab_assem in
  say_if cmd.dump_assem (fun () -> Assem.Print.pp_program assem);
  let codegen_time = Time.now () in
  say_time cmd.verbose "Codegen" translating_time codegen_time;

  (* Optimize *)
  let assem = 
  (match cmd.opt_level with
  | Opt_one -> 
    say_if cmd.verbose (fun () -> "Optimizing...");
    let assem = Optimize.run_all cmd.verbose cmd.debug assem in
    assem
  | _ -> assem
  ) in 
