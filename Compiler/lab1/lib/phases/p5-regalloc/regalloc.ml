...
let available_regs : Reg.t Array.t = 
  Array.of_list Reg.[RBX; RSI; R8; R9; R10; R12; R13; R14; R15]  
;;
...
let regalloc_lines (lines : line list) = 
  ...
  let coloring = (
    if List.length (X.get_nodes graph) < 200
    then (X.color ~inits graph) 
    else Hashtbl.of_alist_exn (module Dest) (List.mapi nodes ~f:(fun i n -> (n, i)))
  )
  in let colors_to_regs = assign_regs coloring in
  (coloring, colors_to_regs)
;;

let regalloc (program : AS.instr list) : AS.instr list =
  let lines = compute_live_outs program in
  let coloring, colors_to_regs = regalloc_lines lines in
  let map_instr' : AS.instr -> AS.instr = map_instr coloring colors_to_regs
  in
    List.map program ~f:map_instr'
;;

let regalloc_checkpoint (input : L.program) : L.allocations = 
  ...
  List.map input ~f:(fun ll -> 
    ...
    ; line = ll.line_number
    ; instr = None})
  in let coloring, colors_to_regs = regalloc_lines lines
  in
