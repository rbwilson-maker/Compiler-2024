...
let conv_binop info op dest_a (lhs, rhs) =
    ...
    match (lhs, rhs) with
      ...
      | _ -> match op with
        | AS.Add -> conv_add info dest (lhs, rhs)
        | AS.Sub -> conv_sub info dest (lhs, rhs)
        | AS.Mul -> conv_mul info dest (lhs, rhs)
        | AS.Div -> conv_div info dest (lhs, rhs)
        | AS.Mod -> conv_mod info dest (lhs, rhs)
    )
...
let convert (l : AS.instr list) =
  ...
  List.fold l ~init:{sops_r = stack_ops_rev; ops_r = ops_rev; table} ~f
   |> fun {sops_r; ops_r; table} -> (* Append leave and return to the end. *)
      {sops_r; ops_r = X.Ret :: (X.Leave :: ops_r); table}
