...
let cmd_line_term : cmd_line_args Cmdliner.Term.t =
  ...
  and alloc = 
    let doc = "Include register allocation phase" in
    flag (Arg.info ["do-regalloc"] ~doc)
  and emit =
...
let regalloc (cmd : cmd_line_args) =
  ...
  | Some base_filename ->
    ...
    let output = Regalloc.regalloc_checkpoint input in
...
let compile (cmd : cmd_line_args) : unit =
  ...
  let ast' = Elaborate.elaborate ast in
  (* Translate *)
  ...
  let assem = 
    if cmd.alloc then Codegen.codegen ir |> Regalloc.regalloc 
    else Codegen.codegen ir in
  ...
  | X86_64 ->
    ...
    Out_channel.with_file file ~f:(fun out -> 
      let output_line line = Out_channel.fprintf out "%s\n" (X86.format line) in
      (match Sys.getenv "UNAME" with
      | Some "Darwin" -> (
        output_line (X86.Directive ("globl \"__c0_main\""));
        output_line (X86.Label ("__c0_main"))
      )
      | _ -> 
        output_line (X86.Directive ("globl \"_c0_main\""));
        output_line (X86.Label ("_c0_main"))
      );
      List.iter ~f:output_line output
    )
;;
