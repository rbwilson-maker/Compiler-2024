...
let rec convert_body (cfg : Cfg.fun_cfg) (instrs : Cfg.fun_lines) (rev_acc : Cfg.fun_lines) : Cfg.fun_lines =
  ...
  | (loc, instr) :: instrs ->
    ...
    | AS.Call c when (not c.extern && List.is_empty c.params && Symbol.equal c.fn cfg.name) ->
      let line_info = Cfg.get_line cfg loc in
      (if line_info.tail_call
      then 
        let tail_label = Symbol.symbol (Symbol.name c.fn ^ "_tail_call") in
        (loc, AS.Jump tail_label) :: rev_acc
      else 
        (loc, instr) :: rev_acc
