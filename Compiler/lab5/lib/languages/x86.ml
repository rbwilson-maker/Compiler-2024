...
module Print = struct
  open Print
  let pp_size : size -> string = Size.format
  ...
  let pp_line : line -> string = function
    ...
    | Leave -> "\tleave"
    | Cltd -> "\tcltd"
    | Jmp label -> sprintf "\tjmp\t%s" (Symbol.name label)
    | Jcc (cc, label) -> sprintf "\tj%s\t%s" (pp_cc cc) (Symbol.name label)
    | Pop reg -> sprintf "\tpopq\t%s" (pp_reg reg Quad)
    | Push op -> sprintf "\tpushq\t%s" (pp_op Quad op)
    | Lea (s, src, dest) -> 
      sprintf "\tlea%s\t%s, %s" (pp_size s) (pp_op s src) (pp_op s dest)
    | Mov (s, src, dest) -> 
