...
and infer_exp ?(void_ok=false) (ast : AM.mexp) (vars : init_status S.t) : AT.mexp =
  ...
  | AM.Fn_call (name, args) ->
    let texp = infer_fn_exp ast vars (name, args) in
    (match (void_ok, T.Tag.untag_opt texp) with
    | (false, None) -> tc_error ~marked:ast "Unexpected void return type."
    | _ -> texp)
...
and infer_fn_exp mark vars (name, args) : AT.mexp =
  ...
  | Some (defn_status, (ret_typ, params)) ->
    ...
    | None ->
      (
        let args = match List.map2 params args ~f:(fun (param_typ, _name) (arg_mexp) ->
        ...
        in
        match ret_typ with
        | Void -> T.Tag.tag_naked (AT.Fn_call (name, args))
        | Typ t -> T.Tag.tag (AT.Fn_call (name, args)) t
      )

let mark_to_tag_asnop (op : AM.asnop) : AT.asnop = match op with
  | Plus_eq -> Plus_eq
  | Minus_eq -> Minus_eq
  | Times_eq -> Times_eq
  | Div_eq -> Div_eq
...
let tc_simple mark s env : fn_env * (AT.simple) = match s with
 ...
 *)
let rec tc_stm (stm : AM.mstm) (env : fn_env) : fn_env * AT.mstm =
...
let tc_fn (ret_type, name, params, stms) : AT.gdecl =
  ...
  else AT.Fdefn (ret_type, name, params, stms)
;;
...
: AT.gdecl =
  ...
  | Fdefn (ret_typ, name, params, body) ->
    ...
    tc_fn (ret_typ, name, params, body) (* Typecheck function body. *)
  | Typedef (typ, name) ->
