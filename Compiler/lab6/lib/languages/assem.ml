...
(* label(%rip) *)
type relative_addr = label
type fn_addr = 
  | Label of label * bool

  (* Value should only ever be a temp or a register, but 
     for register allocation purposes etc, we generalize to operand
   *)
  | Value of operand
...
module Print = struct
  ...
  let pp_instr = function
    ...
    | Call f -> 
      ...
      (pp_operand f.dest)
      (match f.fn with
       | Label (l, _) -> Symbol.name l
       | Value v -> pp_operand v)
    ...
    | Mov_reladdr mv -> sprintf "%s <-- %s(rip)" (pp_operand mv.dest) (Symbol.name mv.src)
    ...
    | Jump l -> "goto " ^ (match l with Label (l, _) -> Symbol.name l | Value v -> pp_operand v)
