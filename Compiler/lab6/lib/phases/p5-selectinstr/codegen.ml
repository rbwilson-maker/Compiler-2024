...
let rec munch_exp_acc ?dest (size : Size.t) (exp : T.exp) (rev_acc : AS.instr list)
  ...
  in
  let make_dest s = match dest with
  | None -> AS.Temp (Temp.create (), s)
  | Some dest -> dest
  in
  match exp with
  | T.Binop binop -> 
  ...
  | T.Fn_name f ->
    let dest = make_dest Quad in
    AS.Mov_reladdr {dest; src = f} :: rev_acc, dest
  | x -> (match dest with 
...
let munch_impure_exp_acc dest (size : Size.t) (imp : T.impure_exp) (rev_acc : AS.instr list)
: AS.instr list
= 
match imp with
| Pure exp -> fst (munch_exp_acc ~dest size exp rev_acc)
| Effect_binop {op; lhs; rhs} ->
  let rev_acc', t1 = munch_exp_acc Long lhs rev_acc in
  let rev_acc'', t2 = munch_exp_acc Long rhs rev_acc' in
  AS.Binop {dest; op = munch_effectop op; lhs = t1; rhs = t2}
  :: rev_acc''
| Fn mv ->
  ...
  let (fn, rev_acc'') : AS.fn_addr * AS.instr list = match mv.fn with
    | `Computed e -> 
      let rev_acc'', e = munch_exp_acc Quad e rev_acc' in
      (match e with
      | AS.Temp _ as x -> Value x, rev_acc''
      | op -> let t = AS.Temp(Temp.create (), Quad) in
        Value t, AS.Mov {dest = t; src = op; size = Quad} :: rev_acc'')
    | `Fn (l, b) -> Label (l, b), rev_acc'
  in
  AS.Call 
    { dest
    ; size = Some size
...
| Read_mem addr ->
  let rev_acc', src_mem = munch_addr_acc addr rev_acc in
  ...
  then (
    ...
    *)
    ...
    let rev_acc'''' = 
      ...
      :: rev_acc''' 
    in
    List.rev_append last_instrs rev_acc''''
    )
| Alloc {elem_size; array_len} ->
  ...
  | Some l -> 
    ...
    :: AS.Branch 
        ...
        ; cond = `Comparison {lhs = dest; rhs = AS.Imm (`Long Int32.zero); op = Neq; size = Quad}}
    :: AS.Call
      { dest
      ; size = Some size
      ; fn = Label (calloc_sym, true)
...
let rec munch_stms_acc (stms : T.stm list) (rev_acc : AS.instr list) : AS.instr list =
  ...
  | T.Move {dest = (t, s); src} :: stms -> 
    let rev_acc' = munch_impure_exp_acc (AS.Temp (t, s)) s src rev_acc in
    munch_stms_acc stms rev_acc'
  | T.Return e :: stms -> (match e with
    | None -> AS.Return :: rev_acc |> munch_stms_acc stms
    | Some (e, s) -> 
      let rev_acc' = munch_impure_exp_acc (AS.Reg RAX) s e rev_acc in
  ...
  | T.Branch b :: stms -> 
    ...
    |> munch_stms_acc stms
  | T.Goto l :: stms -> 
    (* the true is a dummy value indicating function externality and should not be used... *)
    AS.Jump (Label (l, true)) ::rev_acc |> munch_stms_acc stms
