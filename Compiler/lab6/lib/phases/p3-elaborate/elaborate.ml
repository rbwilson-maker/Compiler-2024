...
; header_fns = Hash_set.create (module S)
}
...
| A.Fn_name f -> E.Fn_name (!globl.map_name f)
| A.Const i -> E.Const i
| A.True -> E.True | A.False -> E.False
| A.Lam (typ, params, body) -> 
  let lam_sym = Symbol.unique_symbol "lam" in
  fns_to_elab := A.Fdefn (typ, lam_sym, params, body) :: !fns_to_elab;
  E.Fn_name (!globl.map_name lam_sym)
...
| A.Fn_call (e, exps) -> 
  E.F 
    { name = (
      match T.data e with
      | A.Fn_name f -> `Fn (!globl.map_name f, Hash_set.mem !globl.header_fns f)
      | _ -> `Computed (elab_mexp e)
    )
...
| A.Do (e) -> E.Do
  ...
  | A.Fn_call (e', exps) ->
    (match T.data e' with 
    | A.Fn_name f -> 
      (match (Hashtbl.find_exn !globl.fn_types f) with
      | Type.Void -> 
        E.F 
          { name = `Fn (!globl.map_name f, Hash_set.mem !globl.header_fns f)
          ; params = List.map exps ~f:elab_with_small_size
          ; ret_size = None
          }
      | _ -> elab_mexp e
...
| A.Assert e -> E.Assert (elab_mexp e)
...
let elaborate ~(tc_data : Typechecker.output) : Elab.program = 
  ...
  globl := {
    ...
    header_fns = tc_data.header_decls
  };

  fns_to_elab := (List.filter tc_data.tagged_ast ~f:(function A.Fdefn _ -> true | _ -> false));
  let out = ref [] in
  while (not (List.is_empty !fns_to_elab)) do
    match !fns_to_elab with
    | Fdefn (typ, symb, params, stms) :: rest ->
      fns_to_elab := rest;
      ...
      out := (map_name symb, new_params, new_stms) :: !out;
    | _ -> ()
  done;
  List.rev !out
;;