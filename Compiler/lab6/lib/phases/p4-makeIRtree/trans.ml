...
let rec trans_exp_acc temps (e : E.exp) rev_acc : T.stm list * T.exp =
  match e with
  ...
  | E.Fn_name f -> (rev_acc, T.Fn_name f)
  | E.Binop binop ->
    ...
    | E.Lshift | E.Rshift ->
      ...
      then (
        T.Move 
        { dest 
        ; src = T.Pure (T.Binop { lhs; op = trans_binop binop.op; rhs})
        } :: rev_acc', T.Temp dest
      ) else (
    ...
    | E.Neq -> (rev_acc', T.Equality {lhs; rhs; op = T.Neq; size = binop.op_size})
  ...
  | E.Ternary t ->
    ...
    in let rev_acc''', else_exp = 
      T.Label else_label :: T.Goto after_label :: T.Move {dest; src = T.Pure then_exp} :: rev_acc''
      |> trans_exp_acc temps t.else_exp
    in (
      T.Label after_label :: 
      T.Move {dest; src = T.Pure else_exp} 
      :: rev_acc'''
      , T.Temp dest)
  | E.F f ->
    ...
    let rev_acc', fn = (
        match f.name with 
      | `Computed e -> 
        let acc, e = trans_exp_acc temps e rev_acc in
        T.Check_null e :: acc, `Computed e
      | `Fn f -> rev_acc, `Fn f
    ) in
    let rev_acc'', args' = 
    ...
    ( match dest with
      | None -> 
        T.Void_fn { fn; args } :: rev_acc'',
        T.Temp (Temp.create_with_name "this will not be reached", Word)
      | Some dest ->
        T.Move { dest; src = T.Fn { fn; args }} :: rev_acc'', 
        T.Temp dest
    )
  | E.Raise s -> 
  ...
  | E.Mem (m, s) ->
    ...
    if !unsafe_mode 
    then (
      T.Move {dest; src = T.Read_mem addr} :: rev_acc', T.Temp dest
    ) else (
      let rev_acc'' = 
        if check_null
      ...
      in (T.Move {dest; src = T.Read_mem addr} :: rev_acc'', T.Temp dest)
    )
  | Alloc elem_size ->
    let dest = (Temp.create (), Size.Quad) in
    (T.Move {dest; src = T.Alloc {elem_size; array_len = None}} :: rev_acc, T.Temp dest)
  | Alloc_array (elem_size, e) ->
    let dest = (Temp.create (), Size.Quad) in
    let (rev_acc', e) = trans_exp_acc temps e rev_acc in
    (T.Move {dest; src = T.Alloc {elem_size; array_len = Some e}} :: rev_acc', T.Temp dest)
...
match mem with
  ...
  | E.Deref e ->
    ...
    let (rev_acc', e) = trans_exp_acc temps e rev_acc in
    (match e with 
    | T.Temp (t, _) -> rev_acc', T.Addr_temp t, (not !unsafe_mode)
    | _ -> 
      let addr : Temp.t = Temp.create () in
      (T.Move {dest = (addr, Quad); src = T.Pure e} :: rev_acc'
  ...
  | E.Index {array; index; elem_size} ->
    ...
    (T.Move {dest = (addr, Quad); src = T.Array_addr {array; index; elem_size}}
     :: T.Check_null array
     :: rev_acc''
...
let trans_impure_exp_acc temps (e : E.exp) rev_acc : T.stm list * T.impure_exp = 
  match e with
  | E.Binop b -> (
    match b.op with
    | E.Divided_by | E.Modulo ->
      let (s1, lhs) = trans_exp_acc temps b.lhs rev_acc in
      let (rev_acc', rhs) = trans_exp_acc temps b.rhs s1 in
      (rev_acc', 
      T.Effect_binop 
        { op = trans_effectop b.op
        ; lhs; rhs
        })
    | _ -> let rev_acc', p = trans_exp_acc temps e rev_acc in (rev_acc', T.Pure p)
  )
  | E.F f ->
    let rev_acc', fn = (
        match f.name with 
    ...
    List.fold f.params ~init:(rev_acc', []) ~f:(fun (acc, ps) (e, s) ->
      let stms, p = trans_exp_acc temps e acc in (stms, (p, s)::ps))
    in let args = List.rev args' in
    (match f.ret_size with
    | None -> 
      T.Void_fn { fn; args } :: rev_acc'',
      T.Pure (T.Temp (Temp.create_with_name "this will not be reached", Word))
    | Some _ -> rev_acc'', T.Fn { fn; args }
    )
  | E.Mem (m, _) ->
    let (rev_acc', addr, check_null) = addr_of_memexp temps m rev_acc in
    if !unsafe_mode 
    then (
      rev_acc', T.Read_mem addr
    ) else (
      let rev_acc'' = 
        if check_null
        then (match addr with 
      ...
      in (rev_acc'', T.Read_mem addr)
    )
  | E.Alloc elem_size -> 
    (rev_acc, T.Alloc {elem_size; array_len = None})
  | E.Alloc_array (elem_size, e) ->
    let (rev_acc', e) = trans_exp_acc temps e rev_acc in
    (rev_acc', T.Alloc {elem_size; array_len = Some e})
  | _ -> let rev_acc', p = trans_exp_acc temps e rev_acc in (rev_acc', T.Pure p)
;;
...
let rec trans_lval_acc temps (lval : E.lval) (rev_acc : T.stm list) : T.stm list * T.addr =
  ...
  | E.Deref lval ->
    ...
    then (
      ...
      (T.Move {dest = addr2, Quad; src = T.Read_mem addr1} :: rev_acc'
      , T.Addr_temp addr2)
    ) else (
      let addr2 = Temp.create () in
      (T.Move {dest = addr2, Quad; src = T.Read_mem addr1} :: T.Check_null (T.Addr addr1):: rev_acc'
      , T.Addr_temp addr2)
    )
  | E.Field {strct; field; _} ->
    ...
    ( T.Move
      { dest = addr2, Quad
      ; src = T.Read_mem (T.Field_addr {strct = struct_addr; field})
      }
  ...
  | E.Index {array; index; elem_size} -> 
    ...
    then (
      let addr = Temp.create () in
      T.Move {dest = addr, Quad; src = T.Read_mem (T.Unsafe (T.Array_addr {array = T.Addr array_addr; index; elem_size}))}
      :: rev_acc'', T.Addr_temp addr
    ) else (
      let addr1 = Temp.create () in
      let addr2 = Temp.create () in
      ( T.Move {dest = addr2, Quad; src = T.Read_mem (T.Addr_temp addr1)}
        :: T.Move {dest = addr1, Quad; src = Array_addr  {array = T.Addr array_addr; index; elem_size}}
...
and addr_of_lval temps (lval : E.lval) (rev_acc : T.stm list) : T.stm list * T.addr * bool = 
  ...
  | E.Index {array; index; elem_size} -> 
    ...
    then (
      ...
      let addr = Temp.create () in
      (T.Move {dest = addr, Quad; src = T.Array_addr {array = T.Addr array_addr; index; elem_size}}
      :: T.Check_null (T.Addr array_addr)
      :: rev_acc''
      , T.Addr_temp addr, false)
    )
...
let rec trans_stms_acc (env : Temp.t S.t) (ast : E.stm list) (rev_acc : T.stm list): T.stm list =
  ...
  | E.Return e :: _ ->
    ...
    (match e with 
     ...
     | Some (e, s) ->
    let (rev_acc', imp) = trans_impure_exp_acc env e rev_acc in
    T.Return (Some (imp, s)) :: rev_acc')
  | Block block :: stms -> 
  ...
  | E.Assign ((x, size), e) :: stms ->
    (match x with 
    | E.Var x ->
      let dest = (S.find_exn env x, size) in
      let (rev_acc', imp) = trans_impure_exp_acc env e rev_acc in
      T.Move {dest; src = imp} :: rev_acc'
    | lval -> 
      (* 1) address of lval
         2) translate e
  ...
  | E.Asnop {lhs = (x, s); op; rhs} :: stms ->
    ...
    | E.Var x -> 
      ...
      let (rev_acc', imp) = trans_impure_exp_acc env (
        E.Binop
    ...
    | lval ->
      ...
      let rev_acc''' = 
        ...
        then (match addr with 
          | T.Addr_temp t ->
            T.Move { dest = lhs; src = T.Read_mem addr}
            :: T.Check_null (T.Temp (t, Quad)) 
            :: rev_acc''
          | _ -> failwith "Null should have been checked already"
        )
        (* even in unsafe mode, must enforce eval order *)
        else T.Move {dest = lhs; src = T.Read_mem addr} :: rev_acc''
      in
      (match op with 
      | E.Div_eq | E.Mod_eq ->
        T.Write_mem
  ...
  | E.Assert e :: stms ->
    ...
    :: T.Branch 
      { condition = e
      ; if_label; else_label = Some else_label; after_label}
    :: rev_acc' |> trans_stms_acc env stms
