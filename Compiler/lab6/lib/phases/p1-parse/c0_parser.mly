...
%type <Asts.Marked.typ list> type_list_follow
%type <Asts.Marked.typ list> type_list
...
type_list_follow :
  | (* empty *)
      { [] }
  | Comma; typ = typ; rest = type_list_follow;
      { typ :: rest }

type_list :
  | L_paren; R_paren;
      { [] }
  | L_paren; typ = typ; rest = type_list_follow; R_paren;
      { typ :: rest }
...
typ : 
  ...
  | typ = typ; L_bracket; R_bracket;
      { Type.Array (typ) }
  | inputs = type_list; Arrow; output = ret_type;
      { Type.Arrow (inputs, output) }

ret_type :
  | typ = typ; %prec Low_prec
      { Type.Typ typ }
  | Void;
      { Type.Void }
...
exp :
  ...
  | b = bool;
      { b }
  | Lambda; params = param_list; Arrow; typ = ret_type; body = block;
      { Asts.Marked.Lam (typ, params, body)}
  ...
  | f = m(exp) args = arg_list; 
      { Asts.Marked.Fn_call (f, args) }
  | Null;
      { Asts.Marked.Null }
