...
let update_uses_defs (line : AS.instr) (uses : Dest.t Hash_set.t) = 
  ...
  | AS.Mov_reladdr {dest; _} -> defs_op dest;
  ...
  | AS.Call {params; fn; _} -> (* dest should be RAX *)
    List.iter params ~f:(fun (op, _) -> uses_op op);  
    (match fn with Label _ -> () | Value v -> uses_op v);
  ...
  | AS.Jump l -> (match l with Label _ -> () | Value v -> uses_op v); `None
  | _ -> `None
;;
...
let make_fun_cfg (f : AS.fun_instrs) : fun_cfg = 
  ...
  List.iter rev_body ~f:(fun ((b, curr_sym), line) ->
    ...
    let succ = (
      ...
      | Jump l -> 
        (* jmp *%rax is equivalent to a call, so *) 
        (match l with Label (l, _) -> One (b, l) | _ -> None)
