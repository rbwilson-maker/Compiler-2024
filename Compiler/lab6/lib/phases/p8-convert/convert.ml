...
let conv_operand ?mov_data info = function
  ...
  | AS.Addr m as t -> 
    let unwrap_mov_data () = 
      ...
      | _ -> 
        failwith (Printf.sprintf "conv_operand needs mov_data that was not provided. It was called on this operand: %s" (AS.Print.pp_operand t))
      ) in
...
let conv_lea info (dest, src) = 
  ...
  | `A a ->
    let (info, src_op) = 
      conv_operand ~mov_data:(R11, R14, None) info (AS.Addr a)
    in info, `A src_op
  | `R rel_addr -> info, `R rel_addr
  ) in
  let dest_needs_popping = ref None in
  let (info, dest) = (match dest with
  | AS.Addr a -> 
    (* patterns of this logic are not exactly right, but it's ok for now *)
    if addr_uses_reg a R12
    then conv_operand ~mov_data:(R13, R14, Some dest_needs_popping) info (AS.Addr a)
    else conv_operand ~mov_data:(R12, R14, Some dest_needs_popping) info (AS.Addr a)
  | _ -> conv_operand info dest 
  ) in
  { info with
    ops_r = ( 
    match !dest_needs_popping with
    | Some r -> 
      ( X.Pop r
...
let conv_binop info (op : AS.operation) dest_a (lhs, rhs) =
  ...
  (* dest might be an address calculation -- put dest in R12/13 *)
  let should_pop_dest = ref None in
  let (info, dest) = 
    match dest_a with
    | AS.Addr a -> 
      if addr_uses_reg a R12
      then conv_operand ~mov_data:(R13, R14, Some should_pop_dest) info dest_a 
      else conv_operand ~mov_data:(R12, R14, Some should_pop_dest) info dest_a 
    | _ -> conv_operand info dest_a
  in
  ...
  | None -> info
;;
let conv_fn_addr (info, f) : conversion_info * X.fn_addr = match f with
  | AS.Value op -> 
    (match op with
    | AS.Reg r -> info, Value r
    | _ ->
    let should_pop_dest = ref None in
    let (info, op) = 
    conv_operand ~mov_data:(R11, R12, Some should_pop_dest) info op in 
    (match !should_pop_dest with
     | None -> {info with ops_r = (X.Mov (Quad, op, X.Reg R11)) :: info.ops_r}
     | Some r -> {info with ops_r = 
      (X.Pop r) :: 
      (X.Mov (Quad, op, X.Reg R11)) :: info.ops_r}
      ), Value R11
    )
  | AS.Label (l, _) -> info, X.Label l

let conv_branch info (_if_label, else_label, after_label, cond) =
  match cond with
  | `Single (opand, s) ->
    let (info, opand) = conv_operand info opand in
    let unmet_cond_label : X.fn_addr = match else_label with
      | None -> X.Label after_label
      | Some else_label -> X.Label else_label
    in
    let ops_r = List.rev_append (
      match opand with 
      | X.Imm _ -> 
...
let conv_call info (_dest, fn, spills : AS.operand * AS.fn_addr * (AS.operand * Size.t) list) = 
  ...
  match spills with 
  | [] -> 
    let info, fn = conv_fn_addr (info, fn) in
    {info with ops_r = X.Call (Quad, fn) :: info.ops_r}
  | _::_ -> 
  let ops_r = 
...
| AS.Mov_addr {dest; src} -> conv_lea info (dest, `A src)
| AS.Mov_reladdr {dest; src} -> conv_lea info (dest, `R src)
| AS.Branch {if_label; else_label; after_label; cond} ->
    conv_branch info (if_label, else_label, after_label, cond)
| AS.Jump label ->
  let info, label = conv_fn_addr (info, label) in
  { info with ops_r = (X.Jmp label) :: info.ops_r }
| AS.Label label ->
  { info with ops_r = (X.Label label) :: info.ops_r }
| AS.Call {dest; params; fn; _} -> conv_call info (dest, fn, params)
| AS.Return -> {info with ops_r = X.Jmp (Label info.exit_label)::info.ops_r}
| AS.Directive _ -> info
| AS.Comment _ -> info
| AS.Raise Sigabrt -> {info with ops_r =  X.Call (Quad, Label abort_sym) :: info.ops_r}
| AS.Raise Sigusr -> {info with ops_r = X.Jmp (Label sigusr_label)::info.ops_r}
| AS.Raise Sigfpe -> {info with ops_r = X.Jmp (Label sigfpe_label)::info.ops_r}
;;
...
let convert_function (f : AS.fun_instrs) =
  ...
  |> fun (instr_list, spilled_temp_bytes) ->
    ...
    (X.Directive (sprintf "globl \"%s\"" (Symbol.name f.name))) ::
    (X.Label f.name) ::
    (X.Push (X.Reg RBP)) ::
