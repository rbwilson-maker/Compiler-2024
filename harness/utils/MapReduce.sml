structure MapReduce = struct
  ...
  fun start (mapper : 'a -> value) (x : 'a) =
    let
      ...
      fun fork temp mapper x =
        ...
        end handle exn => OS.Process.exit OS.Process.failure
    in
  ...
  fun join (res as (pid, status)) =
   fn [] => [] (* Pid not found! (Unexpected, possible error) *)
            ...
            let
              ...
              val (worklist, xs) =
                case xs of
                  [] => (join res worklist, [])
                | x :: xs =>
                  (join res worklist @ [ start mapper x ], xs)
            in
              loop fail succeed worklist xs
            end
