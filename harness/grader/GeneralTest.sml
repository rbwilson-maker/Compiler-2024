...
struct
  ...
  fun checks file test lib expected =
    ...
    in
      ...
      , { check = RELAXED (fn () => TestFile.validName base)
        , failMsg = [ "Disallowed characters in filename " ^ file
      ...
      , { check = STRICT (fn () => Option.map TestFile.validExt ext
                                |> Option.isSome)
      ...
      , { check = RELAXED (fn () =>
            not (customLib lib)
              orelse expected = SOME Outcome.ERROR
              orelse expected = SOME Outcome.TYPECHECK)
        , failMsg =
            [ "Bad directive for " ^ file
            , "(tests with custom header files can only test 'error'"
              ^ " or 'typecheck')"
            ]
        }
        (* Here's where we force tests to have the right extensions: make
        * sure an l3 test is not misclassified as an l4 test, etc. *)
      , { check = STRICT (fn () => expected = SOME Outcome.ERROR orelse
            case TestFile.prevExt ext of
              NONE => true (* No good definition of 'previous' *)
            | SOME prevLang =>
                let
                  val () = printqf 0 "\n-- Testing %s at language %s --"
                    [ string test, string prevLang ]
                  val (actual, log) =
                    Compiler.runCC0 (test, lib, ["--standard=" ^ prevLang])
                      Outcome.ERROR
                in
  ...
  fun validateAndMerge (_, acc as SOME _) = acc
    | validateAndMerge ({ check, failMsg }, _) =
        ...
        | RELAXED f =>
            if !Config.relax orelse f () then NONE else SOME failMsg
    ...
    in
      ...
      | NONE =>
        ...
        in
          ...
          ; Either.OK (Summary.summarize { expected = expected, actual = actual })
          )
        end
    end
