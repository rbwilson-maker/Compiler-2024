  ...
  fun one (suiteDir, lib, file) =
    let
      ...
      fun benchmark (test, optimize, safe) =
        let
          val res = Compiler.benchmark (test, optimize, safe)
        in case res of
            Outcome.TIME_OK (cycles, codesize) =>
              ( printq 1 (Util.green (desc (optimize, safe) ^ IntInf.toString cycles))
              ; printq 1 (Util.green ("            Executable size in bytes:" ^ IntInf.toString codesize))
              ; SOME res )
          | Outcome.TIME_INF =>
              (printq 4 (Util.yellow (desc (optimize, safe) ^ "Cycle counting failed: "
              ^ Outcome.toString (Outcome.INFLOOP (SOME Outcome.ALRM))))
              ; SOME res)(* But, it's still ok. *)
        end handle Fail msg =>
         ( printq 4 (Util.red (desc (optimize, safe) ^ "Failed: " ^ msg))
         ; NONE )
...
(*      fun cycMin (NONE,  _) = NONE
    ...
    in case Outcome.readTestDirective test
      of SOME (Outcome.RETURN v) =>
        let
           val () = printq 2 ""
        ...
        in
          case (safe1,unsafe1)
            of (SOME (Outcome.TIME_OK (t1, s1)), SOME (Outcome.TIME_OK (t2, s2))) =>
                SOME (JSON.ARRAY [ JSON.ARRAY [JSON.INT t1,JSON.INT t2]
