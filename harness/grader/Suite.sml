...
struct
  ...
  fun sourceFilesInDir suiteDir =
    let
      val keep =
        ...
        if not (!Config.prune) then (fn _ => true) else

        (* Build a set of pruned files to keep. *)
        let
          val toKeep = Util.fold
            (fn (line, toKeep) => StringSet.add (toKeep, Util.trim line))
            StringSet.empty
            (suiteDir // "keep.txt")
        in
          printqf 1 "-- Found %s/keep.txt file: pruning; Keeping %d tests. --"
            [string suiteDir, int (StringSet.numItems toKeep) ]
          ; (fn file => StringSet.member (toKeep, file))
        end handle exn => (* If there's no keep.txt file, we just keep everything. *)
          ( printqf 1
              ( "-- No %s/keep.txt file found -- "
              ^ "not pruning; all tests will be run --"
              )
              [string suiteDir]
          ; (fn _ => true)
          )
  ...
  fun mapSuite f { suiteName, suiteDir } =
    let
      val (unknownErrorTests, summaries) =
        ...
        ; MapReduce.map
            (E.toValue o E.map TestFile.testErrorToValue S.toValue o f)
       ...
       *)
      val (invalidTestsOpt, goodTests) = E.partition (fn (x, v) =>
          case E.map TestFile.testErrorFromValue S.fromValue (E.fromValue v) of
  ...
  val filter =
    List.mapPartial (fn file =>
      let
        val file = OS.Path.mkCanonical file
        fun fail msg =
          ( printqf 0 (Util.dull "-- %s --") [string msg]
          ; NONE
          )
      in
        case OS.Path.splitDirFile file of
          { dir = "", file = _ } =>
            ...
            then SOME { suiteName = file, suiteDir = ".." // "tests" // file }
              ...
              ( printqf 0
                "Cannot find suite %s or ../tests/%s, ignoring"
        ...
        | { dir = _, file = filepart } =>
            ...
            else fail ("Cannot find suite at " ^ file ^ ", ignoring")
      end
    )
end
