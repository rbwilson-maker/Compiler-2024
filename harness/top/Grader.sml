...
structure Grader :> GRADER = struct
  ...
  fun printNoArgsMsg (default : string list) : string list =
    ( printq 7 ("No args provided; defaulting to looking in directories: "
        ^ String.concatWith ", " default)
    ; printq 7 (
        "To run different tests, provide the test directories as arguments to"
          ^ " the grading harness.")
    ; default
    )
  ...
  fun makeCompiler () =
    ...
    in
      ...
      then printq 2 ("-- Compiler built (elapsed time " ^ elapsedTime ^ "s) --")
  ...
  fun benchCompiler args =
    let
      ...
      val suites =
        case Args.init (String.tokens Char.isSpace args)
          of [] => printNoArgsMsg [ "bench" ]
           | args => args
        ...
        if not (!Config.autograder) then ()
          ...
          JSONPrinter.print
            ...
            , JSON.OBJECT
                ...
                , ("tests",
                      JSON.INT (if Posix.Process.W_EXITED = status then 1 else 0))
                , ("results", JSON.ARRAY [])
                ])
  ...
  fun gradeCompiler args =
    let
      ...
      val timer = makeCompiler ()
      val () =
          if !Config.PARALLELISM <= 1 then () else
       ...
       case Runner.log
         ...
         of Posix.Process.W_EXITED =>
            let
              ...
              val vers =
                List.nth
                  ( String.tokens Char.isSpace (valOf (TextIO.inputLine f))
                  , 5
            ...
            in
              case Int.fromString vers of
                NONE =>
                  failwithf
                    "Could not read %s as a version number (cc0 -V)"
                    [string vers]
              | SOME n =>
                  if n >= Config.MIN_VERSION
                  then printqf 0 "-- Reference compiler: %s version %d --"
                    [ string (!Config.cc0), int n ]
                  else failwithf "Reference compiler version %d < %d"
                    [ int n, int Config.MIN_VERSION ]
            end
          | _ => raise Fail
            ("Reference compiler absent or outdated (" ^ !Config.cc0 ^ ")")

      val () =
        if !Config.PARALLELISM > 1
        then printqf 2 "-- Validating tests using %d cores --"
          [int (!Config.PARALLELISM)]
        else ()
