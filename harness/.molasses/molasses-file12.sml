...
structure Summary :> SUMMARY =
  struct
    ...
    fun catDesc ERR = "typechecker to report an error"
      ...
      | catDesc (OR_LOOP cat) =
        catDesc cat ^ " or nontermination/stack overflow"
    ...
    fun desc (cat, res) =
      sprintf "Expected %s, %s:" [string (catDesc cat), string (resDesc res)]

    fun summarize (outcomes as {expected, actual}) =
      let
        open Outcome
        val result =
          if matches outcomes then
            SUCC
          else
            case actual of
              TIMEOUT => TIME
