...
structure General =
  struct
    ...
    fun maybeTypecheck directive =
      ...
      | other => if ! Config.typecheckOnly then Outcome.TYPECHECK else other
    ...
    fun one (suiteDir, lib, file) =
      let val test = suiteDir // file in
        ...
        | SOME expected =>
            if ! Config.safeOnly andalso not (Outcome.isSafe expected) then
              Either.ERR TestFile.SKIP
            else if ! Config.unsafeOnly andalso Outcome.isSafe expected then
              Either.ERR TestFile.SKIP
            else if
              ! Config.skipFrontendTests andalso Outcome.isFrontendTest expected
            then
              Either.ERR TestFile.SKIP
            else
              let
              ...
              in
                (Outcome.report test log (withActual actual); Either.OK summary)
              end
      end
