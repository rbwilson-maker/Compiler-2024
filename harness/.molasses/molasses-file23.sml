(* grader/Checkpoint.sml : 1.1-104.1 *)
(* molasses-file23.sml *)
local
	infix 5 **
	infix 5 //
...
structure Checkpoint =
  struct
    ...
    fun verifyOneL1 (suiteDir, lib, file) =
      let val test = suiteDir // file in
        case Outcome.readL1CheckpointDirective test of
          NONE =>
            ( printqf 0 (Util.dull "-- Cannot read directive from %s --")
                [string test]
        ...
        | SOME target_score =>
            let
              ...
              val log = NONE
            (* TODO: maybe don't do this -- thea *)
            in
              ( Outcome.report test log {expected = expected, actual = outcome}
              ; Either.OK
                  (Summary.summarize {expected = expected, actual = outcome})
              )
            end
      end
    ...
    fun verifyOneL2 (suiteDir, lib, file) =
      let
        ...
        val refDirs =
          List.map (fn dir => suiteDir ^ "-" ^ dir) (! Config.dataflowArgs)
        val refFiles =
          List.map (fn refDir => refDir // filebase ** SOME "out") refDirs

        fun helper args files =
          case (args, files) of
            ([], []) => Outcome.RETURN 0
          | (arg :: args', file :: files') =>
              if not (Util.isRead file) then
                Outcome.COMPILERFAIL
                  ("Reference output file does not exist: " ^ file)
              else
                let
      ...
      in
        ( Outcome.reportNoDirective test log
    ...
    fun generate1 (suiteDir, lib, file) =
      let
        ...
        val outcome2 =
          case (outcome1, lib) of
            (Outcome.COMPILE, Compiler.NOLIB "l2") =>
              Compiler.generateL2RefOutputs suiteDir file
          | _ => outcome1
      in
        Either.OK
          (Summary.summarize {expected = Outcome.COMPILE, actual = outcome2})
      end

    fun verifySuite (suite as {suiteName, suiteDir}) =
      ( printq 6 (Util.bold ("-- Verifying compiler on " ^ suiteName ^ " --"))
      ; if String.isPrefix "l1" suiteName then
          Suite.map verifyOneL1 suite
        else if String.isPrefix "l2" suiteName then
          Suite.map verifyOneL2 suite
        else
          raise Fail ("Checkpoint verifier not supported for " ^ suiteName)
      )
    ...
    fun generateSuite (suite as {suiteName, suiteDir}) =
      let val checkpointDir = suiteName ^ "-checkpoint" in
        ( printq 6
            (Util.bold ("-- Generate checkpoint input for " ^ suiteName ^ " --"))
        ; if String.isPrefix "l2" suiteName then
            generateL2Suites checkpointDir
          else
            ()
