...
structure Compiler :> COMPILER =
  struct
    datatype lib =
    NOLIB of string | DEFAULT of string | HEADER of string * string
    ...
    fun archive file =
      let val oldfile = file ** SOME "old" in
        ( if OS.FileSys.access (oldfile, []) then
            OS.FileSys.remove oldfile
          else
            ()
        ; if OS.FileSys.access (file, []) then
            OS.FileSys.rename {old = file, new = oldfile}
          else
            ()
        )
      end
    ...
    fun remove file =
      if OS.FileSys.access (file, []) andalso not (! Config.log) then
    ...
    fun prepareTestBase test =
      let
        ...
        val () =
          if Util.isDir logDir then
            ()
          else
            failwithf "Log directory %s not prepared" [string logDir]
    ...
    val runtimeLLVM =
      if OS.FileSys.access ("runllvm.c", []) then
        {inFile = "runllvm.c", outFile = "runllvm.o"}
      else
        runtimeLib

    val toPrecompile =
      if runtimeLLVM = runtimeLib then
        [runtimeNoLib, runtimeLib]
      else
        [runtimeNoLib, runtimeLib, runtimeLLVM]
    ...
    fun runC0CTypechecker test testLog redirect args =
      ...
      in
        ...
        | Posix.Process.W_EXITSTATUS w =>
            if List.exists (fn w' => w = w') (! Config.timeStatus) then
              Outcome.TIMEOUT
            else
              Outcome.ERROR
        | Posix.Process.W_SIGNALED s =>
            if
              List.exists (fn s' => Util.signalEq (s, s')) (! Config.timeSig)
            then
              Outcome.TIMEOUT
            else
              Outcome.ERROR
      ...
      in
        ...
        | Posix.Process.W_EXITSTATUS w =>
            if List.exists (fn w' => w = w') (! Config.timeStatus) then
              raise Overflow
            (* ===> TYPECHECK *)
            else if runningCheckpoint then
              failwithf "Compiler failed to generate .out file and return 0x%x"
                [word8 w]
            else
              failwithf "Compiler returned 0x%x after typechecking successfully"
                [word8 w]
        | Posix.Process.W_SIGNALED s =>
            if
              List.exists (fn s' => Util.signalEq (s, s')) (! Config.timeSig)
            ...
            (* ===> TYPECHECK *)
              ...
              failwithf "Compiler failed to generate .out file and raised 0x%s"
                [string (SysWord.toString (Posix.Signal.toWord s))]
            else
              failwithf "Compiler raised 0x%s after typechecking successfully"
                [string (SysWord.toString (Posix.Signal.toWord s))]
        | Posix.Process.W_STOPPED _ =>
            if runningCheckpoint then
              raise Fail "Compiler stopped when trying to generate .out file"
            else
              raise Fail "Compiler stopped after typechecking successfully"(* Move any files that were plausibly created to log directory *) ;
            ...
            (let
             ...
             in
               (case (res, TextIO.inputLine ins) of
                ...
                | _ => Outcome.RUNFAIL ("Error reading " ^ out)) before TextIO.closeIn
                                                                          ins
             end
               handle _ => Outcome.RUNFAIL ("Error reading " ^ out))
        | Posix.Process.W_EXITSTATUS w => Outcome.RUNFAIL "Bad return code"
        | Posix.Process.W_SIGNALED s =>
            if Util.signalEq (s, Posix.Signal.bus) then
              Outcome.INFLOOP (SOME Outcome.BUS)
            else if Util.signalEq (s, Posix.Signal.segv) then
              Outcome.INFLOOP (SOME Outcome.SEGV)
            else if Util.signalEq (s, Posix.Signal.alrm) then
              Outcome.INFLOOP (SOME Outcome.ALRM)
            else if
              List.exists (fn s' => Util.signalEq (s, s')) (! Config.timeSig)
            then
              Outcome.INFLOOP NONE
            else if Util.signalEq (s, Posix.Signal.abrt) then
              Outcome.ABORT
            else if Util.signalEq (s, Posix.Signal.fpe) then
              Outcome.ARITH
            (* 12 is SIGUSR2 on Ubuntu, but we can't count on 12 being
             * SIGUSR2 everywhere. On OSX, it's a non-standard signal
             * SYS, so we cheat *)
            else if Util.signalEq (s, Posix.Signal.usr2) then
              Outcome.MEMERROR
            else if Util.signalEq (s, Posix.Signal.fromWord 0wxC) then
              Outcome.MEMERROR
            else
              Outcome.RUNFAIL
                ("Unexpected signal 0x" ^ SysWord.toString
                                            (Posix.Signal.toWord s))
    ...
    fun link testLog runtime redirect =
      ...
      in
        ...
        | status =>
            if Util.statusIsAlarm status then
              Outcome.COMPILERFAIL "GCC assembler/linker timed out"
            else
              Outcome.COMPILERFAIL "GCC assembler/linker failed"
      end
    ...
    fun llvm testLog extension arch redirect =
      case Runner.redirect
             { msg =
                 sprintf "-- Running llvm compiler on %s --\n"
      ...
      | status =>
          if Util.statusIsAlarm status then
            Outcome.COMPILERFAIL "LLVM compiler (llc) timed out"
          else
            Outcome.COMPILERFAIL "LLVM compiler (llc) failed"

    fun llvmAndRun testLog extension runtime redirect =
      let val arch = "-march=x86-64" in
    ...
    fun parseScore verifierOutLog =
      let val line = Util.head verifierOutLog in
    ...
    fun verifyL1CheckpointOutput test testLog outfile target_score =
      let
        ...
        val outFD =
          Runner.REDIRECT (Posix.FileSys.creat (verifierOutLog, Util.rw))
        val errFD =
          Runner.REDIRECT (Posix.FileSys.creat (verifierErrLog, Util.rw))
        ...
        val outcome =
          case Runner.redirect
            ...
            Posix.Process.W_EXITED =>
              ...
              let val score = Option.valOf (parseScore verifierOutLog) in
          ...
          | _ =>
              let
                val help_msg =
                  ", check dist/log for .out file generated by compiler"
              in
              ...
              val () =
                printq 0
                  ("-- Running verifier on "
                   ...
                   ^ " --")
            in
              if Util.isRead outfile then
                verifyL1CheckpointOutput test testLog outfile target_score
              else
                Outcome.COMPILERFAIL
      ...
      in
        case runCompilerForCheckpoint test testLog Runner.UNCHANGED args of
          Outcome.COMPILE =>
            ...
            in
              if Util.isRead outfile then
                verifyL2CheckpointOutput testLog outfile refOutfile
              else
                Outcome.COMPILERFAIL
                  ("Compiler ran but did not create " ^ outfile)
            end
    ...
    fun generateFile test targetFile args =
      let
        ...
        val inputFD =
          Runner.REDIRECT (Posix.FileSys.creat (targetFile, Util.rw))
      in
    ...
    fun generateL2RefOutputs suiteDir file =
      let
        ...
        val refDirs =
          List.map (fn dir => checkpointDir ^ "-" ^ dir) (! Config.dataflowArgs)
        val refFiles =
          List.map (fn refDir => refDir // file ** SOME "out") refDirs
        ...
        fun helper args files =
          ...
          | (arg :: args', file :: files') =>
              ...
              in
                if outcome = Outcome.COMPILE then
                  helper args' files'
                else
                  outcome
              end
    ...
    fun runC0C (test, lib, configs) expected =
      let
        ...
        val (redirect, log) =
          if ! Config.quiet = 0 then
            (Runner.UNCHANGED, NONE)
          else
        ...
        fun runConfig output config =
          let
            ...
            (* Lab 6, LLVM output *)
            val optimisation =
              ["-O" ^ (LargeInt.toString (! Config.optimisation))]
            val safe = if ! Config.unsafe then ["--unsafe"] else []
            val allConfig =
              libArgs @ outputGeneratingConfig @ optimisation @ safe @ config
            val outcome =
              case runC0CCompiler test testLog redirect allConfig of
                Outcome.COMPILE =>
                  (if expected = Outcome.COMPILE then
                     Outcome.COMPILE
                   else
                     ...
                     case output of
                       Config.X86_64 =>
                         if Util.isRead (testLog ** SOME "s") then
                           linkAndRun testLog runtime redirect
                         else
                           Outcome.COMPILERFAIL
                             ("Compiler ran but did not create "
                              ^ test
                              ^ ".s")
                     | Config.LLVM =>
                         if Util.isRead (testLog ** SOME "bc") then
                           llvmAndRun testLog "bc" runtimeLLVM redirect
                         else if Util.isRead (testLog ** SOME "ll") then
                           llvmAndRun testLog "ll" runtimeLLVM redirect
                         else
                           Outcome.COMPILERFAIL
                             ("Compiler ran but did not create "
                              ^ test
                              ...
                              ^ ".bc")
                     | Config.EXE =>
                         if Util.isExec (testLog ** SOME "exe") then
                           runExe testLog []
                         else
                           Outcome.COMPILERFAIL
                             ("Compiler ran but did not create executable "
                              ^ test
                              ^ ".exe"))
              | outcome => outcome
          in
        ...
        val outcomes =
          case runC0CTypechecker test testLog redirect libArgs of
            Outcome.TYPECHECK =>
              if tcOnly expected then
                [Outcome.TYPECHECK]
              else
                map (runConfig (! Config.output)) configs
          | outcome => [outcome]
      in
        (Runner.close redirect; destroyTestBase testLog; (outcomes, log))
      end
        ...
        case Runner.log
          ...
          Posix.Process.W_EXITED =>
            (let
               ...
               val _ = TextIO.inputLine outFD
               (* skip the first line of the size output *)
               val codesize =
                 List.nth
             ...
             in
               case IntInf.fromString codesize of
                 NONE =>
                   raise
                     Fail "Reading size of executable from command line failed."
               | SOME i => i before TextIO.closeIn outFD
             end
               handle
                   _ =>
                     raise
                       Fail ("Reading size of executable " ^ exe ^ " failed."))
    ...
    fun benchmark (test, optimize, safe) =
      let
        ...
        val redirect =
          if ! Config.quiet = 0 then
            ...
            Runner.SILENCE
        val optArgs =
          ["-ex86-64", "-O" ^ Int.toString optimize] @ (if safe then
                                                          []
                                                        else
                                                          ["--unsafe"]) @ (List.concat
                                                                             (!
      ...
      in
        ...
        | outcome =>
            raise
              Fail ("Checksum computation failed: " ^ Outcome.toString outcome);
        case runExe testLog [] of
          Outcome.RETURN cyc => Outcome.TIME_OK (cyc, getFileSize testLog)
    ...
    fun runCC0 (test, lib, config) expected =
      let
        ...
        val tcArgs =
          if expected = Outcome.ERROR orelse expected = Outcome.TYPECHECK then
        ...
        val outcome =
          case Runner.redirect
            ...
            Posix.Process.W_EXITED =>
              if expected = Outcome.ERROR then
                Outcome.TYPECHECK
              else if expected = Outcome.TYPECHECK then
                Outcome.TYPECHECK
              else if expected = Outcome.COMPILE then
                Outcome.COMPILE
              else if OS.FileSys.access (exe, [OS.FileSys.A_EXEC]) then
                runExe testLog config
              else
                Outcome.COMPILERFAIL ("cc0 did not create " ^ exe)
          | res =>
              if Util.statusIsAlarm res then
                Outcome.TIMEOUT
              else
                Outcome.ERROR
      in
