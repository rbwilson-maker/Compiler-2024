(* top/StaticAnalysis.sml : 1.1-159.1 *)
(* molasses-file26.sml *)
local
	infix 5 **
	infix 5 //
    ...
    fun run dir {suite, goodTests, badTests} =
      let
          ...
          List.partition
            (fn ((s, _, f), _) =>
               let
                 ...
                 val () =
                   ( Util.mkdir (s ^ "-fixed")
                   ; Suite.prepareLogDirectory
                       (# file (OS.Path.splitDirFile s) ^ "-fixed")
                   )
               in
                 case Runner.redirect
                        { msg =
                            "-- Generating static analysis artifacts for "
                            ^ test
                            ^ " --\n"
                        , command = dir // "analyze-tests"
                        , args = [test, "--fix-c0-bug", (s ^ "-fixed") // f]
        ...
        val (passingTests, duplicateTests) =
          if
            not (! Config.failDuplicateTests orelse ! Config.warnDuplicateTests)
          then
            (passingTests, [])
          else
            let
            ...
            in
              ...
              case Runner.redirect
                     ...
                     , out =
                         Runner.REDIRECT
                           (Posix.FileSys.creat (tempFile, Util.rw))
                     , err = Runner.UNCHANGED
                     } of
                Posix.Process.W_EXITED =>
                  let
                    val (accAccept, accRej) =
                      Util.fold
                        (fn (line, (accAccept, accRej)) =>
                           case StringMap.find (accAccept, line) (* Guard against remove exception *) of
                             NONE => (accAccept, accRej)
                           | SOME _ =>

                               let
                                 val (accAccept', data) =
                                   StringMap.remove (accAccept, line)
                               in
                                 printqf 8
                                   (Util.red
                                   ...
                                   , string
                                       (if ! Config.failDuplicateTests then
                                          "modify"
                                        else
                                          "consider modifying")
                                   ];
                                 ( accAccept'
                                 , StringMap.insert (accRej, line, data)
                                 )
                               end)
                        (testDict, StringMap.empty)
                        tempFile
                    val result as (listAccept, listRej) =
                      ( StringMap.listItems accAccept
                      , StringMap.listItems accRej
                      )
                  in
                    (case (! Config.warnDuplicateTests, listAccept) of
                       (true, _ :: _) =>
                         ( printq 8
                             (Util.green
                                ("These tests aren't duplicates. We've never seen " ^ "them before. Gold star!"))
                         ; List.app
                             (fn ((d, _, f), _) =>
                                printqf 8 (Util.green "\t%s") [string (d // f)])
                             listAccept
                         )
                     | _ => ());
                    result
        ...
        val (passingTests, dodgyTests) =
          if not (! Config.failDodgyTests) then
            (passingTests, [])
          else
            ...
            ; List.partition
                (fn ((s, lib, f), (cat, _)) =>
                   ...
                   cat = Summary.ERR orelse let
                                              val expected =
                                                valOf
                                                  (Outcome.readTestDirective
                                                     (s // f))
                                              val (actual, _) =
                                                ...
                                                Util.withRefSetTo
                                                  ...
                                                  (3 * ! Config.COMPILER_TIMEOUT)
                                                  (fn () =>
                                                     Util.withRefSetTo
                                                       Config.RUN_TIMEOUT
                                                       (2 * ! Config.RUN_TIMEOUT)
                                                       (fn () =>
                                                          Compiler.runCC0
                                                            ( (s ^ "-fixed") // f
                                                            , lib
          ...
          in
            failingTests @ map
                             (if ! Config.failDuplicateTests then
                                ...
                                fn x => x)
                             duplicateTests @ map reject dodgyTests
          end
      in
        { suite = suite
