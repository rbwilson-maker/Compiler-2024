...
structure Util :> UTIL =
  struct
    ...
    fun mkdir dir =
      (if not (OS.FileSys.access (dir, [])) then
         OS.FileSys.mkDir dir
       else if not (OS.FileSys.isDir dir) then
         failwithf "Exists but is not a directory: `%s`" [string dir]
       else
         ())
        handle
            exn =>
              failwithf "Error making directory %s: %s"
                [string dir, string (exnMessage exn)]
    ...
    val rw = let open Posix.FileSys.S in flags [irusr, iwusr, irgrp, iwgrp] end
    ...
    fun ls dir =
      let
        ...
        fun loop NONE set = set
          | loop (SOME file) set =
            loop (nextFile ())
              (if shouldSkip (dir // file) then
                 set
               else
                 StringSet.add (set, file))

      in
        StringSet.listItems (loop (nextFile ()) StringSet.empty) before OS.FileSys.closeDir
                                                                          dirstream
      end
    ...
    fun app f file =
      ...
      in
        (loop ())
          handle exn => (TextIO.closeIn stream handle _ => (); raise exn)
      end
    ...
    fun mv {from, to} =
      if isRead from then
        OS.FileSys.rename {old = from, new = to}
      else
        ()
  end
