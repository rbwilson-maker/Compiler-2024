  ...
  struct
    ...
    fun addArgs args =
      let
        ...
        fun appendToEnd arg = if arg = "" then id else (fn args => args @ [arg])
      in
        Config.compilerArgs := concatMap
                                 (fn arg => map (appendToEnd arg) current)
                                 new
      end
    ...
    fun addDataflowArg arg s =
      let val args = ! Config.dataflowArgs in
        if List.length args = 4 then
          Config.dataflowArgs := [arg]
        else
          Config.dataflowArgs := args @ [arg]
      end
    ...
    val options =
      ...
      , { short = "m"
        , long = ["make"]
        , desc =
            GetOpt.ReqArg (fn arg => Config.make := SOME (SOME arg), "<lab>")
      ...
      , { short = ""
        ...
        , desc =
            GetOpt.ReqArg (fn s => Config.staticAnalysisDir := SOME s, "dir")
        , help =
      ...
      , { short = ""
        , long = ["limit-filename"]
        , desc =
            intArg ("n", fn i => Config.MAX_FILENAME_BASE := Int.fromLarge i)
    ...
    val usage = GetOpt.usageInfo {header = "411 Autograder", options = options}
    ...
    fun init args =
      ...
      ; Config.unsafe := false
      ; Config.timeSig := [ Posix.Signal.ill
                          , (* Rust compiler, possibly remove -rjs F15 *) Posix.Signal.bus
                          , (* Stack overflow on Ubuntu/Autograder *) Posix.Signal.segv
                          , (* Stack overflow on OSX/RHEL, sometimes Ubuntu *) Posix.Signal.alrm
                          , (* Autograder in normal behavior *) Posix.Signal.kill
                          ]
      (* Backup kill signal *)
      ; Config.timeStatus := [0wx8]
      (* SBCL student solution, probably remove -rjs F15 *)
      ; let
          val (_, result) =
            GetOpt.getOpt
        ...
        in
          if
            ! Config.failDodgyTests andalso not
                                              (isSome
                                                 (! Config.staticAnalysisDir))
          then
            failwith "--fail-dodgy-tests requires a static analysis dir"
          else
            ();
          if
            (! Config.failDuplicateTests orelse ! Config.warnDuplicateTests) andalso not
                                                                                       (isSome
                                                                                          (!
                                                                                             Config.staticAnalysisDir))
          then
            failwith
              "--fail-duplicate-tests (and --warn-duplicate-tests) requires a static analysis dir"
          else
            ();
          result
        end
