...
structure Config =
  struct
    val MAX_TEST_LIMIT : int = 255
    (* Database limit, don't change for F15 *)
    val MIN_VERSION : int = 522
    (* Minimum correctly configured cc0 vers *)
    ...
    val cc0 = ref "cc0"
    val dataflowArgs =
      ref ["forward-may", "forward-must", "backward-may", "backward-must"]
    ...
    val relax = ref false
    (* Relaxed validation of test cases *)
    val log = ref true
    (* Keep all log files *)
    val compilerArgs : string list list ref = ref [[]]
    (* Extra arguments *)
    val filterExt : string option ref = ref NONE
    (* Test filter *)
    val autograder = ref false
    (* Print autograder output *)
    ...
    val allowInfloopTests : bool ref = ref false
    (* whether infloop tests are allowed *)
