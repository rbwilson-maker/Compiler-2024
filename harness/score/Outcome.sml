...
struct
  ...
  datatype outcome =
    ...
    | RETURN of IntInf.int (* Returns a 32-bit integer from main *)
    | RUNFAIL of string (* Impossible for this to be correct *)
    | OR_INFLOOP of outcome (* Either infloops or has outcome. *)
  type t = outcome
  ...
  fun orInfloop res =
    let
      val canMeaningfullyBeCombinedWithInfLoop =
        ...
        | _ => false
    in
      if canMeaningfullyBeCombinedWithInfLoop
      then SOME (OR_INFLOOP res)
      else NONE
    end
  ...
  fun reportHelper test logfile { expected, actual } hasDirective =
    ...
    in
      ...
      then printqf 1 (Util.green "-- PASS: %s --") [string test]
      else
        case logfile of
          NONE => ()
        | SOME file =>
            ...
            ; if hasDirective
              then (printqf 3 (Util.red "   directive: %s") [string (Util.head test)])
              else ()
            ; printqf 3 (Util.red "     outcome: %s") [string msg]
            )
    end

  fun report test logfile summary =
    reportHelper test logfile summary true
