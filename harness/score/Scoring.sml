...
struct
  fun failure (): unit =
    ...
    else JSONPrinter.print (TextIO.stdOut, JSON.NULL)
  ...
  fun benchmarks stats =
     if not (!Config.autograder) then () else
     let
       val v =
  ...
  fun tests (stats: Compiler.lib Stats.result) =
    let
      ...
      fun catSize f =
        let
          val listWithCat = List.filter (fn (_, (cat, _)) => f cat) allGoodTests
        in
          length listWithCat
        end
    ...
    in
      JSON.OBJECT
        ...
        , ("ret", INT' (catSize (fn cat =>
            cat = Summary.RET orelse cat = Summary.OR_LOOP Summary.RET
            ))
          )
        , ("raise", INT' (catSize (fn cat =>
              cat = Summary.RAISE orelse cat = Summary.OR_LOOP Summary.RAISE
            ))
          )
        , ("header", INT' <| length
            ( List.filter (
