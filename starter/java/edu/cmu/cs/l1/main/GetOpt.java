...
class Option {
 public
  String m_longname, m_shortname, m_description;
 public
  boolean m_data, m_required;
}
...
public class GetOpt {
 ...
 public
  void procCommandLine(String[] argv, int numnonopts) throws Exception {
    int i, nos = 0;
    ...
    while (tokens.hasNext()) {
      ...
      if (curtoken.charAt(0) != '-') {
        ...
        while (reps.hasMoreElements()) {
          ...
          if (("-" + rep.m_longname).equals(curtoken) ||
              rep.m_shortname.equals(curtoken)) {
            inserted = true;
            if (rep.m_data) {
              if (!tokens.hasNext()) {
              ...
              m_datatable.put(key, "Blank");
                     ...
                     curtoken.substring(0, rep.m_shortname.length())
                         .equals(rep.m_shortname)) {
            inserted = true;
            curdata = curtoken.substring(rep.m_shortname.length());
            m_datatable.put(key, curdata);
          }
        }
        if (!inserted) {
 ...
 private
  void passOverReqedOpts() throws Exception {
 ...
 private
  Iterator getTokenIterator(String[] argv) {
    int i;
    Vector v = new Vector();
 ...
 public
  int regOption(String longrep, String shortrep, String description,
                boolean data, boolean required) {
    Option rep = new Option();

    rep.m_longname = longrep;
    rep.m_shortname = shortrep;
    rep.m_description = description;
    rep.m_data = data;
    rep.m_required = required;
 ...
 public
  boolean getOption(int key) {
    return (m_datatable.get(new Integer(key)) != null);
  }

 public
  String getOptionData(int key) {
    return (String)m_datatable.get(new Integer(key));
  }

 public
  String getNonOptionArguments() { return m_nonopt; }

 private
  Hashtable m_keytable = new Hashtable(), m_datatable = new Hashtable();
 private
  String m_nonopt;
 private
  int m_nextKey = 0;
