...
public
class ASPrintAbsyn implements Visitor {
   ...
   */
 public
  ASPrintAbsyn(java.io.PrintStream o) {
    out = o;
    indentLevel = 0;
  }
 ...
 public
  void visitResponse(String type, Object obj) {
    if (type.equals("ASSimpleLval")) {
      ...
      System.out.println(
          "Severe Internal Compiler Warning: Visit passing unknown " +
          "visitable!: " +
          type);
    }
  }
 ...
 private
  void visitIntConstant(Object i) {
  ...
  /** Print out the hex constant */
 private
  void visitHexConstant(Object i) {
    ASHexConstant ah = (ASHexConstant)i;
 ...
 private
  ...
  /** Print out the declaration statement */
 private
  void visitDecl(Object d) {
    ASDeclStatement ds = (ASDeclStatement)d;

    indent();
    if (ds.hasAssignment()) {
      sayln("Decl(" + ds.getVar().toString() + ",");
      indentLevel++;
      ds.getExpression().visit(this);
      indentLevel--;
      indent();
      sayln(")");
    } else {
