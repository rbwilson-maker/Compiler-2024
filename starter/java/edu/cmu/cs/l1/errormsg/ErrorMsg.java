...
class ErrorMsg {
 ...
 public
  void newfile(String fname, int startline) {
 ...
 private
  LineEntry findPositionLine(int pos) {
    int i, j;
 ...
 public
  List getAllFiles() {
    ...
    for (i = 0; i < m_files.size(); i++) {
      ...
      if (!files.contains(fe.m_filename)) files.add(fe.m_filename);
    }
    return files;
  }

 public
  void status() { System.out.println("m_linenum: " + m_linenum); }

 private
  List m_files = new ArrayList();
 private
  int m_linenum = 0, m_lpos = 0;
 private
  String m_filename;
 public
  boolean anyErrors;
